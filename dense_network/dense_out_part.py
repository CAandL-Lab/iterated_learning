...
def dynamic_formula(n1, n2, k1, k2, r, a_init, rho, num_time_steps, num_svds):
  ...
  d = 1/d_inv
  hit_time1 = (tau/(2*s[0]))*np.log((s[0]*rho-d[0]*a_init[0][0]*rho)/(s[0]*a_init[0][0]-d[0]*a_init[0][0]*rho))
  conv_time1 = (tau/(2*s[0]))*np.log((s[0]**2-s[0]*d[0]*a_init[0][0]-s[0]*d[0]*rho+d[0]**2*a_init[0][0]*rho)/(d[0]**2*a_init[0][0]*rho))
  ...
  return sv_trajectory_plots, predicted_sys_norm, predicted_non_norm, quad_norms, U_preds, V_T_preds, sv_indices, hit_time1, conv_time1
...
if __name__ == "__main__":
  ...
  # Data Hyper-parameters 3,2,3,1,2
  ...
  for i in range(num_generations):
      ...
      if i == 0:
          predictions[:,:,i], preds_sys_norms[:,i], preds_non_norms[:,i], preds_quad_norms, U, VT, sv_indices, hit_time1, conv_time1 =\
                                     dynamic_formula(n1, n2, k1, k2, r, a.reshape(1,num_svds), 0.005, num_epochs, num_svds)
  ...
  # Rows are time-steps, Columns are full trajectories, Depth is generations
  ...
  plt.grid()
  plt.savefig('d_norms_plot.pdf')
  plt.savefig('d_norms_plot.png',dpi=400)
  plt.close()
  ...
  plt.ylabel("Frobenius Norm")
  plt.xlabel("Epoch number")
  plt.legend(loc='upper left', bbox_to_anchor=(-0.02, 0.45, 0.5, 0.5),ncol=2)
  plt.grid()
  plt.savefig('d_sys_quad_norms_plot.pdf')
  plt.savefig('d_sys_quad_norms_plot.png',dpi=400)
  plt.close()
  ...
  plt.grid()
  plt.savefig('d_non_quad_norms_plot.pdf')
  plt.savefig('d_non_quad_norms_plot.png',dpi=400)
  plt.close()
  ...
  plt.savefig('d_quad_norms_plot.png',dpi=400)
  plt.close()
  ...
  plt.axhline(0, color='black')
  plt.axvline(0, color='black')
  plt.axvline(hit_time1, color='black', linestyle='dashed')
  plt.axvline(conv_time1, color='black', linestyle='dashed')
  plt.legend(loc='center', bbox_to_anchor=(0.5, 0.79), ncol=2)
  plt.savefig("dSVs.pdf")
  plt.savefig("dSVs.png",dpi=400)
  plt.close()
