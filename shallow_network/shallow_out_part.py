...
def dynamic(n1, n2, k1, k2, r, X, Y, U, VT, a_init, num_time_steps, num_svds, sv_indices):
  ...
  sv_ratio = final_svds_s_only*d_inv
  sv_traj = (final_svds_s_only*d_inv)*(1-np.exp(-taus/d_inv)) + a_init*np.exp(-taus/d_inv)
  sv_trajectory_plots = np.zeros(sv_traj.shape)
  num_sig_svs = sv_ratio[sv_ratio > 0].shape[0]
  sv_trajectory_plots = sv_traj[:,:num_sig_svs]
...
if __name__ == "__main__":
  ...
  for j in range(trainings.shape[1]):
    for k in range(trainings.shape[2]):
        ...
        plt.legend()
    plt.title("Deep Dynamics of Singular Value " + str(j))
  ...
  for k in range(sys_norms.shape[1]):
      if k == 0:
          ...
          plt.plot(non_norms[:,k], color='blue', label=r'$Y\Gamma$-Norm')
  ...
  plt.grid()
  plt.savefig('sh_norms_plot.pdf')
  plt.savefig('sh_norms_plot.png',dpi=400)
  plt.close()
  ...
  plt.grid()
  #plt.savefig('sh_sys_quad_norms_plot.pdf')
  plt.savefig('sh_sys_quad_norms_plot.png',dpi=400)
  plt.close()
  #np.savetxt('shallow_norm_v_gen.txt', non_norms[-1,:])
  ...
  plt.legend(loc='upper left', bbox_to_anchor=(0.0, 0.52, 0.5, 0.5))
  plt.grid()
  #plt.savefig('sh_non_quad_norms_plot.pdf')
  plt.savefig('sh_non_quad_norms_plot.png',dpi=400)
  plt.close()
  ...
  plt.savefig('sh_quad_norms_plot.png',dpi=400)
  plt.close()
