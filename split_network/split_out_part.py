...
def startup(n1,n2,k1,k2,r):
  ...
  norms = np.zeros((num_epochs, num_generations))
  preds_norms = np.zeros((num_epochs, num_generations))
  quad_norm_1 = np.zeros((num_epochs, num_generations))
  quad_norm_2 = np.zeros((num_epochs, num_generations))
  pred_quad_norm_1 = np.zeros((num_epochs, num_generations))
  pred_quad_norm_2 = np.zeros((num_epochs, num_generations))
...
if __name__ == "__main__":
  ...
  for i in range(num_generations):
      ...
      sys_a = np.ones(sys_num_svds)*8e-7
      non_a = np.ones(non_num_svds)*8e-7
      sys_traj_real = sys_a
      non_traj_real = non_a
      ...
      if i == 0:
          ...
          sys_predictions[:,:,i], preds_sys_norms[:,i], _, preds_sys_quad_norms =\
                  dynamic(n1, n2, k1, 0, r, X, Y[:n2], sys_U, sys_VT, sys_a.reshape(1,sys_num_svds), num_epochs, sys_num_svds, sys_svd_indices)
          preds_sys_sys_norms[:,i] = preds_sys_quad_norms[0]
          preds_non_sys_norms[:,i] = preds_sys_quad_norms[1]
          non_predictions[:,:,i], _, preds_non_norms[:,i], preds_non_quad_norms =\
                  dynamic(n1, 0, k1, k2, r, X, Y[n2:], non_U, non_VT, non_a.reshape(1,non_num_svds), num_epochs, non_num_svds, non_svd_indices)
          preds_sys_non_norms[:,i] = preds_non_quad_norms[2]
          preds_non_non_norms[:,i] = preds_non_quad_norms[3]
      ...
      sys_net_labels = predict(sys_params, X)
      non_net_labels = predict(non_params, X)
      Y = np.vstack([sys_net_labels, non_net_labels])
      sys_full_map = np.dot(sys_params[1], sys_params[0])
      non_full_map = np.dot(non_params[1], non_params[0])
      _, sys_s, _ = svs(sys_full_map, sys_U, sys_VT, sys_num_svds, 0)
      _, non_s, _ = svs(non_full_map, non_U, non_VT, non_num_svds, k2)
      print("Generation "+str(i)+" Systematic SVs: ", sys_s)
      print("Generation "+str(i)+" Non-Systematic SVs: ", non_s)
      sys_trainings[:,:,i] = sys_traj_real[1:]
      non_trainings[:,:,i] = non_traj_real[1:]

  # Plot each trajectory from each network module.
  # Row are time-steps, Columns are full trajectories
  print("Num Plots: ", sys_trainings.shape[2])
  for j in range(sys_trainings.shape[1]):
    for k in range(sys_trainings.shape[2]):
        if k == 0:
            plt.plot(np.arange(num_epochs), sys_predictions[:,j,k], color='red', label='Prediction', linestyle='dashed')
            plt.plot(np.arange(num_epochs), sys_trainings[:,j,k], color='blue', label='Real')
        else:
            plt.plot(np.arange(num_epochs), sys_predictions[:,j,k], color='red', linestyle='dashed')
            plt.plot(np.arange(num_epochs), sys_trainings[:,j,k], color='blue')
        plt.legend()
    #plt.title("Deep Dynamics of Systematic Singular Value " + str(j))
    plt.ylabel("Singular Value")
    plt.xlabel("Epoch number")
    plt.grid()
    plt.axhline(0, color='black')
    plt.axvline(0, color='black')
    #plt.savefig(str(j)+"_sys.pdf")
    plt.close()

  print("Num Plots: ", non_trainings.shape[2])
  for j in range(non_trainings.shape[1]):
    for k in range(non_trainings.shape[2]):
        if k == 0:
            plt.plot(np.arange(num_epochs), non_predictions[:,j,k], color='red', label='Prediction', linestyle='dashed')
            plt.plot(np.arange(num_epochs), non_trainings[:,j,k], color='blue', label='Real')
        else:
            plt.plot(np.arange(num_epochs), non_predictions[:,j,k], color='red', linestyle='dashed')
            plt.plot(np.arange(num_epochs), non_trainings[:,j,k], color='blue')
        plt.legend()
    ...
    plt.axhline(0, color='black')
    plt.axvline(0, color='black')
    #plt.savefig(str(j)+"_non.pdf")
    plt.savefig(str(j)+"_non.png",dpi=400)
    plt.close()
  ...
  for k in range(sys_norms.shape[1]):
      if k == 0:
          plt.plot(sys_norms[:,k], color='red', label=r'$Y_\Omega$-Norm')
          plt.plot(non_norms[:,k], color='blue', label=r'$Y_\Gamma$-Norm')
          plt.plot(preds_sys_norms[:,k], color='green', label=r'Predicted $Y_\Omega$-Norm', linestyle='dashed')
          plt.plot(preds_non_norms[:,k], color='orange', label=r'Predicted $Y_\Gamma$-Norm', linestyle='dashed')
      else:
  ...
  plt.xlabel("Epoch number")
  plt.legend(loc='upper left', bbox_to_anchor=(0.0, 0.52, 0.5, 0.5),ncol=2)
  plt.grid()
  plt.savefig('s_norms_plot.pdf')
  plt.savefig('s_norms_plot.png',dpi=400)
  plt.close()
  #np.savetxt('split_norm_v_gen.txt', non_norms[-1,:])

  # Plot Systematic Quadrant norms
  for k in range(sys_norms.shape[1]):
      if k == 0:
          plt.plot(sys_sys_norms[:,k], color='red', label=r'$X_\Omega Y_\Omega$-Norm')
          plt.plot(preds_sys_sys_norms[:,k], color='green', label=r'Predicted $X_\Omega Y_\Omega$-Norm', linestyle='dashed')
          plt.plot(non_sys_norms[:,k], color='blue', label=r'$X_\Gamma Y_\Omega$-Norm')
          plt.plot(preds_non_sys_norms[:,k], color='orange', label=r'Predicted $X_\Gamma Y_\Omega$-Norm', linestyle='dashed')
      else:
  ...
  plt.grid()
  #plt.savefig('s_sys_quad_norms_plot.pdf')
  plt.savefig('s_sys_quad_norms_plot.png',dpi=400)
  plt.close()

  # Plot Non-sys Quadrant norms
  for k in range(sys_norms.shape[1]):
      if k == 0:
          plt.plot(sys_non_norms[:,k], color='purple', label=r'$X_\Omega Y_\Gamma$-Norm')
          plt.plot(preds_sys_non_norms[:,k], color='lime', label=r'Predicted $X_\Omega Y_\Gamma$-Norm', linestyle='dashed')
          plt.plot(non_non_norms[:,k], color='navy', label=r'$X_\Gamma Y_\Gamma$-Norm')
          plt.plot(preds_non_non_norms[:,k], color='cyan', label=r'Predicted $X_\Gamma Y_\Gamma$-Norm', linestyle='dashed')
      else:
  ...
  plt.legend(loc='upper left', bbox_to_anchor=(0.0, 0.52, 0.5, 0.5))
  plt.grid()
  #plt.savefig('s_non_quad_norms_plot.pdf')
  plt.savefig('s_non_quad_norms_plot.png',dpi=400)
  plt.close()
  ...
  plt.savefig('s_quad_norms_plot.png',dpi=400)
  plt.close()
  ...
  for j in [unique_indices[0], unique_indices[2], unique_indices[1]]:
      ...
      for k in range(trainings.shape[2]):
          if k == 0:
              ...
              plt.plot(np.arange(num_epochs), trainings[:,j,k], color=identifier[1])
  for j in [unique_indices[0], unique_indices[2], unique_indices[1]]:
      identifier = colours[np.round(predictions[-1,j,0],2)]
      labelr = identifier[2]
      for k in range(predictions.shape[2]):
          if k == 0:
              ...
              plt.plot(np.arange(num_epochs), predictions[:,j,k], color=identifier[0], linestyle='dashed')
  ...
  plt.axhline(0, color='black')
  plt.axvline(0, color='black')
  plt.legend(loc='center', bbox_to_anchor=(0.5, 0.79),ncol=2)
  plt.savefig("sSVs.pdf")
  plt.savefig("sSVs.png",dpi=400)
  plt.close()
  ...
  plt.imshow(start_labels.T)
  plt.xticks([])
  plt.yticks([])
  plt.savefig('init_labels.pdf',dpi=400)
  plt.close()
  plt.imshow(Y.T)
  plt.xticks([])
  plt.yticks([])
  plt.savefig('final_labels.pdf',dpi=400)
  plt.close()
